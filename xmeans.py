...
class XMeans:
    ...
    class Cluster:
        ...
        def log_likelihood(self):
            loglike = 0
            for x in self.data:
                if np.ndim(self.cov) != len(self.center):
                    cov = self.cov + 0.0001 * np.identity(len(self.center))
                    loglike += stats.multivariate_normal.logpdf(x, self.center, cov)
                else:
                    loglike += stats.multivariate_normal.logpdf(x, self.center, self.cov)
